...
public class LogManagerImpl implements LogManager {
    ...
    public void appendEntries(List<LogEntry> entries, final StableClosure done) {
        ...
        writeLock.lock();
        try {
            // checkAndResolveConflict检查日志是否冲突
            // 比如logIndex<appliedIndex 或 logIndex>lastIndex+1等
            ...
            for (int i = 0; i < entries.size(); i++) {
                ...
                //如果是“配置变更”类型的消息，把新旧量配置都加入到configManager中，进行配置变更
    ...
    private class AppendBatcher {
        ...
        LogId flush() {
            if (this.size > 0) {
                this.lastId = appendToStorage(toAppend);//写入磁盘
